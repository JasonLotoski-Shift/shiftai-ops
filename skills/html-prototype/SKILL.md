...
- **Light + dark themes, with a toggle.** Every prototype ships TWO coherent palettes — a full light version and a full dark version of its design — plus a clearly visible theme-toggle button in the top bar/header (a sun/moon icon or a compact "Light / Dark" control). Drive theming with CSS custom properties on `:root`, switched by a `data-theme="light|dark"` attribute (or class) on `<html>`, so one variable set powers each mode — never hard-code a color that only reads in one theme. **Both** modes must hold the contrast bars from the design principles (≥4.5:1 body) and keep the brand accent recognisable in each. **The default (first-load) theme follows the brand's palette character:** if the brand direction skews dark / deep / moody — or it's the `[Shift Edition-06 fallback]` — open in **dark**; if the brand skews light / airy / clean, open in **light**. The toggle flips instantly (no reload) and persists the choice in `localStorage`.
- **Header** names the client and reads, plainly, that this is an illustrative prototype built by Shift.
- **Self-QA before returning:** every tab switches and every interaction works, **both the light and dark themes render with adequate contrast and the theme toggle flips and persists**, nothing is lorem/placeholder, no `TODO`, tables have realistic volume, the file renders standalone with no network beyond fonts, and no banned words appear.

...
5. **What you own** — frame it as ownership, plainly. The client **owns the custom system you build** (the Deliverable) outright once it is paid for, with its own data and a runnable version it keeps in a repo it controls. The firm owns and keeps the reusable engine underneath it (the Background IP: patterns and framework, never the client's data), which the client licenses for the monthly fee. One line on what it's built on for reassurance: a modern, maintainable web stack the firm runs and supports, with the client's data in its own database, kept plain, never a jargon dump.
...
11. **Investment** — the one-time **build fee** (which buys the custom build outright) **and** the monthly **Background IP licence fee** for the engine that keeps it running and improving, **only if supplied** (see rules). Otherwise `[NEEDS INPUT]`. Fixed engagement fee, never hourly. There is no buy-out: the client owns the build; the monthly fee is for the Background IP, not a path to buying it.
...
- **No hourly billing.** The firm bills on a fixed engagement fee/value plus the monthly Background IP licence fee, never hours.

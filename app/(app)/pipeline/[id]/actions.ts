...
import { drive, seedClientSubfolders, uploadAsGoogleDoc, brandGoogleDoc } from "@/lib/drive";
...
export async function convertDeal(
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // ── Deal-source commission carry-forward → unified CommissionLine (Phase 4) ──
    // Each deal commission becomes a source CommissionLine on the new project
    // (basis = build value). A subscription with a 6/12-month base also spawns a
    // ServiceContract beginning on a FUTURE date (the build's target end); that
    // line carries the recurring % + covered months. Payouts (build + recurring)
    // are generated by the shared recompute AFTER the billing schedule exists, so
    // the carried commission matches an MP-added line to the dollar (§9.6).
    const spawnsOngoing =
    ...
    let commissionLinesCreated = 0;
    for (let i = 0; i < sourceCommissions.length; i++) {
      const row = sourceCommissions[i];
      // Recurring applies only to a 6/12-month base on a spawned service contract.
      const recurring = spawnsOngoing && row.base !== "deal_value";
      const coveredMonths = recurring ? (row.base === "total_12mo" ? 12 : 6) : null;
      await tx.commissionLine.create({
        data: {
          ...
          basis: "build_value",
          buildPct: row.pct,
          recurringPct: recurring ? row.pct : null,
          coveredMonths,
          onSchedule: true,
          sortOrder: i,
          ...
          notes: row.notes,
          // Provenance: this line was carried from a deal commission at convert.
          backfillSourceTable: "DealSourceCommission",
          backfillSourceId: row.id,
        },
      });
      commissionLinesCreated++;
    }
    ...
    // Generate commission payouts now that the billing schedule exists — the
    // build stream splits each source line across the installments, the recurring
    // stream lays out one payout per covered month against the service contract.
    if (commissionLinesCreated > 0) {
      await recomputeCommissionPayoutsTx(tx, project.id);
      await recomputeRecurringCommissionPayoutsTx(tx, project.id);
    }
...
export async function saveContract(dealId: string, input: { body: string }) {
  ...
  try {
    await brandGoogleDoc(fileId, { clientName: deal.company });
  } catch (e) {

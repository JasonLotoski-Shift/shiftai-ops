...
// Project-level deal-source commission editor — the fallback for when a deal
// converted without a commission set (or to adjust the snapshot). Mirrors the
// deal editor; shows the computed one-time build-slice dollars. 6/12-month bases
// are only offered when the project has an on-going service contract (else the
// recurring slice has nothing to apply to). Managing-partner gated.
...
import {
  addProjectSourceCommission,
  deleteProjectSourceCommission,
} from "@/app/(app)/projects/[id]/billing-actions";

export type ProjectCommissionBaseValue = "deal_value" | "total_6mo" | "total_12mo";

export type ProjectCommissionRow = {
  id: string;
  partnerId: string | null;
  externalName: string | null;
  partnerName: string;
  pct: number;
  base: ProjectCommissionBaseValue;
  buildAmount: number;
  notes: string | null;
};
...
const BASE_LABELS: Record<ProjectCommissionBaseValue, string> = {
  deal_value: "Deal value (one-time build)",
  total_6mo: "6-month total",
  total_12mo: "12-month total",
};

export function ProjectSourceCommissionEditor({
  projectId,
  rows,
  partners,
  allowRecurringBase = false,
}: {
  projectId: string;
  rows: ProjectCommissionRow[];
  partners: CommissionPartner[];
  allowRecurringBase?: boolean;
}) {
      ...
      <CardBody className="flex flex-col gap-3 pt-0">
        {rows.length === 0 && !adding && (
          <p className="text-[11px] text-bone-mute">
            No commission carried from the deal. Add a partner or outside referrer here if someone sourced this work.
          </p>
        )}
          ...
          <div key={r.id} className="flex items-center justify-between gap-3 py-1">
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[11px] text-bone-mute">
                {BASE_LABELS[r.base]} · {r.pct}%
              </span>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              <span className="mono text-[12px] text-track-gold tabular-nums">{formatCAD(r.buildAmount).replace("CA$", "$")}</span>
              <button
                onClick={() => run(() => deleteProjectSourceCommission(r.id))}
                disabled={isPending}
        ...
        {adding && (
          <CommissionForm
            partners={partners}
            allowRecurringBase={allowRecurringBase}
            isPending={isPending}
            onCancel={() => setAdding(false)}
            onSubmit={(v) => run(() => addProjectSourceCommission(projectId, v), () => setAdding(false))}
          />
        )}
...
function CommissionForm({
  ...
  isPending: boolean;
  onCancel: () => void;
  onSubmit: (v: { partnerId?: string; externalName?: string; pct: number; base: ProjectCommissionBaseValue }) => void;
}) {
  const [payeeKind, setPayeeKind] = useState<"partner" | "external">("partner");
  const [partnerId, setPartnerId] = useState(partners[0]?.id ?? "");
  const [externalName, setExternalName] = useState("");
  const [pct, setPct] = useState("5");
  const [base, setBase] = useState<ProjectCommissionBaseValue>("deal_value");

  const bases: ProjectCommissionBaseValue[] = allowRecurringBase
    ? ["deal_value", "total_6mo", "total_12mo"]
    : ["deal_value"];
  ...
  return (
    <div className="flex flex-col gap-2 p-3 border border-graphite rounded-[var(--radius)]">
      ...
      <div className="flex items-center gap-2">
        <Select value={base} onChange={(e) => setBase(e.target.value as ProjectCommissionBaseValue)} className="h-8 text-[13px] flex-1">
          {bases.map((b) => (

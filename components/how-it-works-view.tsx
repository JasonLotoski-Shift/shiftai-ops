...
const flows: Flow[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { tone: "claude", kind: "Claude", label: "Label a bill or payment email “ops-AR/AP” in Gmail and the review sorts it four ways: a vendor bill we owe → “Add to AP” (a duplicate invoice number is blocked); a receipt someone paid on their OWN card → “Reimburse” (pick the partner or contractor — tracked as owed to them); a receipt already paid on a firm card → “Log firm-paid” (a record only); a payment on an invoice WE sent → “Mark paid” (matches the existing invoice, never a second record). USD converts to CAD automatically and keeps the original USD on the record. A link-only email is flagged “needs detail” with the link" },
      ...
      { tone: "claude", kind: "Claude", label: "Drop or paste an invoice or receipt straight into Ingest and Claude files it the same way a labeled Gmail bill is. It comes up on a green finance card with the PDF or photo shown inline, every figure editable before you file (vendor, amount, currency, invoice number, due date), a project picker (or leave it firm level for overhead like Anthropic, Vercel, or Workspace), and a reimburse list covering everyone on the team, partner or contractor" },

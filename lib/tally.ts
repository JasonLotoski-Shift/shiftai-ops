...
// ── The structured question shape + tolerant parser live in lib/survey.ts
// (prisma/drive-free so the discovery chain + its test can import them). Imported
// here for the block mapping below, and re-exported so existing `@/lib/tally`
// importers (tally-actions, the modal) are unchanged. ──
import { type SurveyQuestion, type SurveyQuestionType } from "@/lib/survey";
export { parseQuestions } from "@/lib/survey";
export type { SurveyQuestion, SurveyQuestionType } from "@/lib/survey";

...
- **Notes:** New 2026-06-18. The skill is scoped to Schedule A on purpose, so the LLM never rewrites the binding clauses, keeping the legal text stable between contracts. The v3 template was **reviewed and approved by BC counsel (2026-06-18)** and the model **ratified by Jay**; dispute resolution is binding arbitration in Vancouver. `[NEEDS INPUT]` still marks facts that block the save (fees, parties, dates, and Shift's own legal details in `firm-party.ts`).
